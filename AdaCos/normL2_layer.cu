...
void NormL2Layer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i=0; i<n; ++i) {
    ...
    caffe_gpu_scale<Dtype>(d, pow(normsqr + 1e-7, -0.5), bottom_data+i*d, top_data+i*d);
    //caffe_gpu_scale<Dtype>(d, pow(normsqr, -0.5), bottom_data+i*d, top_data+i*d);
  }
}

template <typename Dtype>
void NormL2Layer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int i=0; i<n; ++i) {
    ...
    caffe_gpu_scale(d, Dtype(pow(a + 1e-7, -0.5)), bottom_diff+i*d, bottom_diff+i*d);
    //caffe_gpu_scale(d, Dtype(pow(a, -0.5)), bottom_diff+i*d, bottom_diff+i*d);
  }
}

INSTANTIATE_LAYER_GPU_FUNCS(NormL2Layer);
